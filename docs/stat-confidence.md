...
| `touch.touch_count`, `touch.control_touch_count`, `touch.medium_hit_count`, `touch.hard_hit_count`, `touch.aerial_touch_count`, `touch.high_aerial_touch_count`, `touch.wall_touch_count`, `touch.is_last_touch`, `touch.last_touch_time`, `touch.last_touch_frame`, `touch.time_since_last_touch`, `touch.frames_since_last_touch`, `touch.last_ball_speed_change`, `touch.average_ball_speed_change`, `touch.max_ball_speed_change`, `touch.total_ball_travel_distance`, `touch.total_ball_advance_distance`, `touch.total_ball_retreat_distance`, labeled `touch.touch_count` by kind, height, surface, and dodge state | Touches are grounded in replay events, but labels and ball-effect metrics depend on nearby frame state and thresholds. |
...
| `whiff.whiff_count`, `whiff.beaten_to_ball_count`, `whiff.grounded_whiff_count`, `whiff.aerial_whiff_count`, `whiff.dodge_whiff_count`, `whiff.is_last_whiff`, `whiff.last_whiff_time`, `whiff.last_whiff_frame`, `whiff.time_since_last_whiff`, `whiff.frames_since_last_whiff`, `whiff.last_closest_approach_distance`, `whiff.best_closest_approach_distance`, `whiff.average_closest_approach_distance` | Whiffs are inferred from a committed approach, proximity, speed, dodge/aerial state, and clear separation without a touch. Any touch interrupts the whiff candidate; opponent wins are classified by the dedicated `beaten_to_ball` stream. The legacy `whiff.beaten_to_ball_count` remains for playback compatibility. False positives are possible in congested or fake-challenge plays. |
...
| Identifiers | Basis and caveats |
| --- | --- |
| `flick.count`, labeled `flick.count` by `confidence_band` and `kind`, `flick.high_confidence_count`, `flick.is_last_flick`, `flick.last_flick_time`, `flick.last_flick_frame`, `flick.time_since_last_flick`, `flick.frames_since_last_flick`, `flick.last_confidence`, `flick.average_confidence`, `flick.best_confidence`, `flick.average_setup_duration`, `flick.average_ball_speed_change` | Specialized flick detector using setup, timing, ball-speed change, and car/ball state heuristics. Reverse kind labels require backflip-direction pitch, forward impulse relative to dodge-start orientation, and car rotation under the ball; they are heuristic review signals. |
...
| `wall_aerial.count`, `wall_aerial.high_confidence_count`, `wall_aerial.is_last_wall_aerial`, `wall_aerial.last_wall_aerial_time`, `wall_aerial.last_wall_aerial_frame`, `wall_aerial.time_since_last_wall_aerial`, `wall_aerial.frames_since_last_wall_aerial`, `wall_aerial.last_confidence`, `wall_aerial.average_confidence`, `wall_aerial.best_confidence`, `wall_aerial.average_setup_duration`, `wall_aerial.average_takeoff_to_touch_time`, `wall_aerial.average_touch_height` | Specialized detector for controlled wall-to-air continuations. It depends on wall-contact thresholds, inferred wall control, takeoff timing, touch attribution, and ball impulse. |
| `wall_aerial_shot.count`, `wall_aerial_shot.high_confidence_count`, `wall_aerial_shot.is_last_wall_aerial_shot`, `wall_aerial_shot.last_wall_aerial_shot_time`, `wall_aerial_shot.last_wall_aerial_shot_frame`, `wall_aerial_shot.time_since_last_wall_aerial_shot`, `wall_aerial_shot.frames_since_last_wall_aerial_shot`, `wall_aerial_shot.last_confidence`, `wall_aerial_shot.average_confidence`, `wall_aerial_shot.best_confidence`, `wall_aerial_shot.average_takeoff_to_shot_time`, `wall_aerial_shot.average_shot_height` | Specialized detector for shots after wall takeoff. It does not require prior controlled wall possession and depends on replay-reported shot events. |
...
| `dodge_reset.count`, `dodge_reset.on_ball_count` | Tracks raw dodge-refresh counter events and the on-ball subset used as flip-reset evidence. Needs more replay-backed validation before production use. |
