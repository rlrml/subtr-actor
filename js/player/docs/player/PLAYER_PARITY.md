...
The goal: make `ReplayPlayer` API-compatible with [`@rlrml/player`](../../player)'s
...
high-fidelity player unchanged.
...
2. **Shared data layer** _(done — see below)_: `player.replay: ReplayModel` from
...
`PlayerState` matches `ReplayPlayerState` field-for-field:
...
| `customCameraSettings`                        | ✅ live              | delegated to the camera plugin as explicit overrides (they win over the recorded preset). `pitch` is accepted as an alias of the player-native `angle`                                 |
...
| `ballCamEnabled`                              | ✅ live              | reports the **effective** ball-cam state. Until explicitly set, the player follows the replay's recorded per-player ball-cam state (richer than @rlrml/player's static default)        |
...
| `ReplayPlayer`                                                                                                                           | `ReplayPlayer`      | Notes                                                                                                                                                                                                                                                                                           |
| ---------------------------------------------------------------------------------------------------------------------------------------- | ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| `setState(patch)` / `getState()` / `getSnapshot()` / `subscribe()`                                                                       | ✅ same             | `PlayerStatePatch` has the same keys as `ReplayPlayerStatePatch`                                                                                                                                                                                                                                |
...
| `setCameraViewMode("free" \| "follow")`                                                                                                  | ✅                  | `"free"` only leaves follow mode — it won't stomp player-native fly/ballOrbit modes set on the plugin handle                                                                                                                                                                                    |
...
`player.replay` is `@rlrml/player`'s `ReplayModel` — the exact structure its
consumers (most importantly `js/stat-evaluation-player`) read. One WASM parse
feeds both layers: `createPlayer` calls `loadReplay` (`adapter/wasm.ts`), which
returns `loadReplayFromBytes`'s `{ replay, raw }`; `raw` builds the adapter,
`replay` is stored on the player. The model costs nothing extra —
...
- **Time axis.** Raw replay clocks don't start at 0; `normalizeReplayData`
  ...
  events, and exposes `adapter.rawStartTime`. `player.currentTime`,
...
(and the WASM-side fields the player needs), so the package depends on the
...
`PlayerPluginContext` now carries everything `ReplayPlayerPluginContext` does —
...
and the scrubber all drive the player through the parity methods
(`projectTimelineTimeToReplay`, `getTimelineCurrentTime`, `seek`, …).
`context.scene` is the player's `sceneState` (below).
...
`player.sceneState` matches @rlrml/player's `ReplayScene` shape — the surface
...
`(-fieldScale, fieldScale, fieldScale)` scale in its Z-up world; the player
...
`fieldScale = 1` (the player renders 1:1 UU; `options.fieldScale` is absent,
...
`replayRoot`, `resize` are the player's own; `dispose` maps to
`player.destroy()`; `ballMesh` and `playerMeshes` (keyed by stable player id)
...
All of `ReplayPlayerOptions`' `initial*` fields are accepted (`PlayerOptions`):
`initialPlaybackRate` (wins over player-native `speed`),
...
plus the inert toggles. Player-native options (`loop`, `effects`, `autoplay`,
`plugins`) are unchanged. `fieldScale` is **not** supported: this renderer works
1:1 in Unreal Units.

## Known semantic differences

- **Player identity.** @rlrml/player keys everything on `track.id`; the player
...
- **Ball cam default.** @rlrml/player defaults `ballCamEnabled` to a static
  `false`; the player defaults to the _recorded_ per-player ball-cam state and
...
- **Camera settings naming.** @rlrml/player's `pitch` = player's `angle`; both
