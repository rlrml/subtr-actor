...
> **Status:** the contract below is **implemented**. The host is `ReplayPlayer`
> (`src/ReplayPlayer.ts`): a bare core owning scene/arena/actors + the playback clock,
> dispatching these hooks. Types live in `src/types.ts`; the public entry is
> `createPlayer()` (`src/lib.ts`). Built-in plugins so far:
...
interface PlayerPlugin {
  id: string;
  setup?(ctx: PlayerPluginContext): void; // install: attach meshes/DOM, subscribe
  onStateChange?(ctx: PlayerPluginStateContext): void; // play/pause/seek/camera changes
  beforeRender?(ctx: PlayerRenderContext): void; // per-frame, after positions resolve
  teardown?(ctx: PlayerPluginContext): void; // uninstall: dispose everything created
}

type PlayerPluginFactory = () => PlayerPlugin;
type PlayerPluginDefinition = PlayerPlugin | PlayerPluginFactory;

interface PlayerPluginContext {
  player: ReplayPlayer; // the core; exposes seek/play/pause/state + the adapter
...
interface PlayerRenderContext extends PlayerPluginContext {
...
```ts
new ReplayPlayer(container, bytes, {
...
"Hooks" = which `PlayerPlugin` hook a re-implementation would primarily use. "Needs" =
