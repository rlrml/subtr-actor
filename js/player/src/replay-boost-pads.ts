...
interface BoostPadProgressTracker {
  advance(units?: number): boolean;
}

interface AsyncBoostPadProgressTracker extends BoostPadProgressTracker {
...
function bucketBoostPadEvents(
  raw: RawReplayFramesData,
  progressTracker?: BoostPadProgressTracker,
): Map<string, RawBoostPadEvent[]> {
  ...
  return eventsByPadId;
}

async function bucketBoostPadEventsAsync(
  raw: RawReplayFramesData,
  progressTracker: AsyncBoostPadProgressTracker,
): Promise<Map<string, RawBoostPadEvent[]>> {
  const eventsByPadId = new Map<string, RawBoostPadEvent[]>();
...
function buildReplayBoostPadEvents(
  rawEvents: RawBoostPadEvent[],
...
function buildBoostPadsFromRaw(
  raw: RawReplayFramesData,
  players: ReplayPlayerTrack[],
  startTime: number,
  eventsByPadId: Map<string, RawBoostPadEvent[]>,
): ReplayBoostPad[] {
  const playersById = buildPlayerLookup(players);
  const sortedPads = [...(raw.boost_pads ?? [])].sort((left, right) => left.index - right.index);
  ...
  for (let index = 0; index < sortedPads.length; index += 1) {
    ...
    pads[index] = {
      ...
      events: buildReplayBoostPadEvents(rawEvents, playersById, startTime),
    };
  }
...
export function buildBoostPads(
  ...
  const eventsByPadId = bucketBoostPadEvents(raw, progressTracker);
  if (!raw.boost_pads || raw.boost_pads.length === 0) {
    progressTracker?.advance(STANDARD_SOCCAR_BOOST_PAD_COUNT);
    return buildStandardSoccarBoostPads();
  }

  const pads = buildBoostPadsFromRaw(raw, players, startTime, eventsByPadId);
...
export async function buildBoostPadsAsync(
  ...
  const eventsByPadId = await bucketBoostPadEventsAsync(raw, progressTracker);
  if (!raw.boost_pads || raw.boost_pads.length === 0) {
