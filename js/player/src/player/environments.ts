...
const BUILTIN_ENVIRONMENTS: Record<string, PlayerEnvironment> = {
 ...
 */
export function registerEnvironment(env: PlayerEnvironment): void {
 ...
 */
export function resolveEnvironment(spec: PlayerEnvironmentSpec): PlayerEnvironment | null {
  if (spec === false) return null;
  if (typeof spec === "string") {
    const env = BUILTIN_ENVIRONMENTS[spec];
    if (!env) {
      console.warn(`[player] unknown environment "${spec}"; using neutral default`);
      return null;
    }
    return env;
  }
