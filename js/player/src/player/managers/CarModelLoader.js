...
export class CarModelLoader {
  constructor(options = {}) {
    ...
    this.HITBOX_DIMENSIONS = {
      ...
      dominus: { length: 127.9268, width: 83.28, height: 31.3, offsetX: 9.0, offsetZ: 15.75 },
  ...
  async preloadModelsForReplay(players) {
    ...
    console.log(
      `[CarModelLoader] Preloading ${modelTypesArray.length} car models for replay: ${modelTypesArray.join(", ")}`,
    );
  ...
  async _loadModelInternal(modelType) {
    ...
    if (config.format === "glb") {
      // Load GLB file (textures are embedded). Resolve against the player asset
      ...
      const glbPath = resolvePlayerAssetUrl(`${basePath}/${config.file}`, this.assetBase);
      ...
      const fbxPath = resolvePlayerAssetUrl(`${basePath}/${config.file}.fbx`, this.assetBase);
      const modelName = config.file;
      const texturePath = resolvePlayerAssetUrl(
    ...
    model.traverse((child) => {
      ...
      for (const socketName of expectedSockets) {
        if (name === socketName || nameLower === socketName.toLowerCase()) {
          ...
          console.log(
            `   Found socket: "${name}" at position (${child.position.x.toFixed(2)}, ${child.position.y.toFixed(2)}, ${child.position.z.toFixed(2)})`,
          );
        }
      }
    });
    ...
    model.traverse((child) => {
      if (child.isMesh) {
        ...
        materials.forEach((mat, index) => {
          console.log(
            `    [${index}] Material: "${mat.name}" - Color: #${mat.color?.getHexString() || "none"}`,
          );

          const matName = (mat.name || "").toLowerCase();
          const meshName = (child.name || "").toLowerCase();
          const isBodyMaterial = bodyMaterialNames.some(
            (name) => matName.includes(name) || meshName.includes(name),
          );
          ...
          if (format === "glb") {
            ...
            if (mat.isMeshStandardMaterial || mat.isMeshPhysicalMaterial) {
              console.log(
                `      → GLB material (keeping as-is): metalness=${mat.metalness?.toFixed(2)}, roughness=${mat.roughness?.toFixed(2)}`,
              );
    ...
    carMesh.traverse((child) => {
      if (child.isMesh) {
        ...
        materials.forEach((mat, idx) => {
          console.log(
            `    [${idx}] Material: "${mat.name}", isBodyMaterial: ${mat.userData?.isBodyMaterial}`,
          );
        });
      }
    });
      ...
      if (child.isMesh) {
        ...
        materials.forEach((mat, index) => {
          ...
          if (isBodyMaterial) {
            ...
            console.log(
              `🎨 Applied team color to: "${mat.name}" on mesh "${child.name}" (index ${index})`,
            );
          }
        });
      }
  ...
  _attachWheelsToSockets(carModel, wheelModelTemplate) {
    ...
    for (const [socketName, socketInfo] of Object.entries(socketMapping)) {
      ...
      console.log(
        `   ✓ Attached wheel to ${socketName} (${socketInfo.position} ${socketInfo.side})`,
      );
  ...
  _findWheelMeshes(model) {
    ...
    model.traverse((child) => {
      ...
      if (match) {
        ...
        console.log(
          `   Found: "${child.name}" (${axis === "y" ? "wheel mesh" : "steering pivot"})`,
        );
      }
    });
    ...
    for (const [pos, parts] of Object.entries(wheelParts)) {
      ...
      if (wheelMesh) {
        ...
        console.log(
          `🛞 Wheel ${pos.toUpperCase()}: mesh="${wheelMesh.name}"${steeringPivot && info.position === "front" ? `, steering="${steeringPivot.name}"` : ""}`,
        );
      }
    }
