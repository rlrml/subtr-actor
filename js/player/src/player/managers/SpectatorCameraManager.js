/**
 * SpectatorCameraManager - Manages 3D visualization of other viewers' cameras in collaborative sessions
 *
 * Features:
...
export class SpectatorCameraManager {
  /**
  ...
  async loadModel() {
    ...
    this.modelLoading = new Promise((resolve, reject) => {
      console.log("[SpectatorCameraManager] Loading camera drone model...");
      this.gltfLoader.load(
        resolvePlayerAssetUrl("models/camera/camera_drone.glb", this.assetBase),
        (gltf) => {
          console.log("[SpectatorCameraManager] Loaded camera drone model successfully");
        ...
        (progress) => {
          if (progress.total > 0) {
            console.log(
              `[SpectatorCameraManager] Loading: ${Math.round((progress.loaded / progress.total) * 100)}%`,
            );
          }
        },
        (error) => {
          console.error("[SpectatorCameraManager] Failed to load camera model:", error);
  ...
  createLabel(nickname, color) {
    ...
    console.log(
      `[SpectatorCameraManager] createLabel: nickname=${nickname}, inputColor=${color}, bgColor=${bgColor}`,
    );
    ...
    console.log(
      `[SpectatorCameraManager] Created label sprite for ${nickname} with color ${labelColor}`,
    );
  ...
  async addOrUpdateViewer(participantId, participant) {
    ...
    if (!viewerData) {
      ...
      if (this.modelTemplate) {
        ...
        // Use fallback mesh
        console.log("[SpectatorCameraManager] Using fallback mesh");
      ...
      if (viewerData.mesh.visible !== !shouldBeHidden) {
        ...
        console.log(
          `[SpectatorCameraManager] *** Visibility change: ${nickname} -> ${!shouldBeHidden ? "VISIBLE" : "HIDDEN"}`,
        );
      }
    }
   ...
   * @param {string} participantId - ID of the player who is following
  ...
  removeViewer(participantId) {
    ...
    console.log(`[SpectatorCameraManager] Removed camera for ${participantId}`);
  }
  ...
  updateFromParticipants(participants, selfId) {
    ...
    for (const id of newHiddenFollowers) {
      if (!this.hiddenFollowers.has(id)) {
        ...
        console.log(
          `[SpectatorCameraManager] *** HIDING follower ${viewerData?.nickname || id} (mesh exists: ${!!viewerData})`,
        );
    ...
    for (const id of this.hiddenFollowers) {
      if (!newHiddenFollowers.has(id) && id !== this.hiddenViewerId) {
        console.log(`[SpectatorCameraManager] Participant ${id} stopped following - showing`);
