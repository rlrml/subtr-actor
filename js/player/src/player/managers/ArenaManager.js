...
export class ArenaManager {
  constructor(scene, options = {}) {
    ...
    this.dracoLoader.setDecoderPath(resolvePlayerAssetUrl("draco/", this.assetBase));
  ...
  async loadDrawingCollider(visible = false) {
    try {
      ...
      const collider = await objLoader.loadAsync(
        resolvePlayerAssetUrl("models/stadium/DrawingArena.obj", this.assetBase),
      );
  ...
  async loadArenaDecor(show = true) {
    try {
      ...
      const gltf = await this.gltfLoader.loadAsync(
        resolvePlayerAssetUrl("models/stadium/arene.glb", this.assetBase),
      );
