  ...
  for (let i = 0; i < FIREBALL_POOL_SIZE; i++) {
    const mat = new THREE.SpriteMaterial({
      ...
      color: new THREE.Color().setHSL(
        0.08 - (i / FIREBALL_POOL_SIZE) * 0.08,
        1,
        0.5 + (i / FIREBALL_POOL_SIZE) * 0.3,
      ),
      transparent: true,
      blending: THREE.AdditiveBlending,
      depthWrite: false,
...
function createObjectPools() {
  ...
  for (let i = 0; i < fireballCount; i++) {
    ...
    const mat = new THREE.SpriteMaterial({
      map: glowTexture,
      color: new THREE.Color().setHSL(
        0.08 - (colorIndex / 40) * 0.08,
        1,
        0.5 + (colorIndex / 40) * 0.3,
      ),
      transparent: true,
      blending: THREE.AdditiveBlending,
      depthWrite: false,
  ...
  const debrisGeos = [
    _objectPools.geometries.tetra,
    _objectPools.geometries.box,
    _objectPools.geometries.octa,
  ];
  ...
  update(deltaTime) {
    for (const explosion of this.explosions) {
      ...
      for (let i = 0; i < explosion.rays.length; i++) {
        const ray = explosion.rays[i];
        const rayLength =
          rayBaseLength * rayPulse * (0.8 + Math.sin(i * 0.5 + explosion.elapsed * 8) * 0.2);
...
class BallTrail {
  ...
  emit(position, velocity) {
    ...
    if (!this.active) {
      console.log(
        `✨ Ball trail activated! Speed: ${speed.toFixed(2)}, Team: ${this.team === 0 ? "Blue" : "Orange"}`,
      );
    }
  ...
  update(delta) {
    ...
    this.trailPoints = this.trailPoints.filter(
      (point) => this.gameTime - point.time < this.trailLifetime,
    );
...
class TrailSegment {
  ...
  createTrail(targetObject) {
    ...
    trail.initialize(material, this.trailLength, false, 0, headGeometry, targetObject);
 ...
 */
// Lazily-built shared geometry cache for DemolitionExplosion (created once,
// reused across explosions). Mirrors the shapes/dims in the simplified pool's
// `geometries` block; `debris` cycles three convex hulls for chunk variety.
let _demoGeometryCache = null;
function getGeometryCache() {
  if (!_demoGeometryCache) {
    _demoGeometryCache = {
      ringH: new THREE.RingGeometry(10, 50, 64),
      ringV: new THREE.RingGeometry(10, 40, 64),
      debris: [
