...
export function setPlayerAssetBase(assetBase) {
  configuredAssetBase = assetBase == null ? null : String(assetBase);
}

export function getPlayerAssetBase(assetBase = configuredAssetBase) {
  const base = assetBase == null || assetBase === "" ? viteBaseUrl() : String(assetBase);
  return new URL(base, documentBaseUrl()).href;
}

export function resolvePlayerAssetUrl(path, assetBase = configuredAssetBase) {
  const rawPath = String(path);
  if (isAbsoluteUrl(rawPath)) {
    return rawPath;
  }
  return new URL(rawPath.replace(/^\/+/, ""), getPlayerAssetBase(assetBase)).href;
}
