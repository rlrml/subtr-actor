 ...
 * PlayerPlugin hooks and fed by the adapter's `boostPads` map (which carries
...
export function createBoostPadsPlugin(): PlayerPlugin {
  let boostPadMeshes = new Map<number, PadMesh>();

  function createBoostPads(ctx: PlayerPluginContext): void {
  ...
  function updateBoostPads(ctx: PlayerRenderContext): void {
    const boostPads = ctx.player.adapter.boostPads;
