 ...
 *   createPlayer(container, bytes, { plugins: [cam] });
...
// CameraSettings lives in types.ts (shared with ReplayPlayer's parity surface);
...
export function createCameraPlugin(options: CameraPluginOptions = {}): CameraPlugin {
  ...
  let pluginCtx: PlayerPluginContext | null = null;
  ...
  function updateCamera(ctx: PlayerRenderContext, dt: number): void {
    if (!manager) return;
    if (mode === "free") {
