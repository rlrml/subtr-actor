 ...
 * original Ballcam viewer flashed on every goal.
 *
 ...
 * glow text-shadow. Opt in like any other `ViewerPlugin`, and toggle at runtime
 * via `viewer.addPlugin(...)` / `viewer.removePlugin("scored-text")`:
 *
 *   createViewer(container, bytes, { plugins: [createScoredTextPlugin()] })
 *
...
export function createScoredTextPlugin(options: ScoredTextOverlayOptions = {}): ViewerPlugin {
  ...
  const resolveMount = (ctx: ViewerPluginContext): HTMLElement =>
  ...
  return {
    ...
    setup(ctx): void {
      ...
      root.className = "viewer-scored-text-overlay";
      root.style.cssText = `
      ...
      banner.className = "viewer-scored-text";
