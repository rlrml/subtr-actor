...
export class ActorManager {
    ...
    initInterpolants(timelines) {
        ...
        console.log(`  Ball: ${this.ballTimeline.length} keyframes (${correctedBallCount} corrected, ${filteredBallCount} filtered)`);
        Object.entries(this.playerTimelineMap).forEach(([name, timeline]) => {
    ...
    _createAnimationClip(name, timeline, target) {
        ...
        if (firstKeyframe.time > 0) {
            ...
            if (firstKeyframe.position) {
                positions.push(firstKeyframe.position.x, firstKeyframe.position.y, firstKeyframe.position.z);
            } else {
    ...
    _findKeyframeIndex(timeline, time, lastIndex = 0) {
        ...
        if (idx + 2 < timeline.length && timeline[idx + 1].time <= time && timeline[idx + 2].time > time) {
            return idx + 1;
        }
    ...
    _applyAdaptiveSmoothing(entityId, pos, time) {
        ...
        const speed = Math.sqrt(
            state.derivedVel.x ** 2 +
            state.derivedVel.y ** 2 +
            state.derivedVel.z ** 2
        );
        ...
        if (state.buffer.length >= 2) {
            ...
            if (prevMag > 0.1 && currMag > 0.1) {
                const dot = (prevVel.x * currVel.x + prevVel.y * currVel.y + prevVel.z * currVel.z) /
                           (prevMag * currMag);
    ...
    _velocitySmoothInterpolate(entityId, k0, k1, currentTime, isBall = false) {
        ...
        const errorMagnitude = Math.sqrt(
            totalError.x * totalError.x +
            totalError.y * totalError.y +
            totalError.z * totalError.z
        );
    ...
    _smartHybridInterpolate(k0, k1, currentTime) {
        ...
        if (speed0 > 10 && speed1 > 10) {
            dotProduct = (k0.velocity.x * k1.velocity.x + k0.velocity.y * k1.velocity.y + k0.velocity.z * k1.velocity.z) / (speed0 * speed1);
        }
        ...
        const isCollision = dotProduct < 0.95 || speedChangePct > 0.10;
    ...
    _physicsSimInterpolate(k0, k1, currentTime, isBall = false) {
        ...
        return {
            x: h00 * k0.position.x + h10 * (k0.velocity.x * dt) + h01 * k1.position.x + h11 * (k1.velocity.x * dt),
            y: h00 * k0.position.y + h10 * m0y + h01 * k1.position.y + h11 * m1y,
            z: h00 * k0.position.z + h10 * (k0.velocity.z * dt) + h01 * k1.position.z + h11 * (k1.velocity.z * dt)
        };
    }
    ...
    getPlayerPositionAt(playerName, time) {
        ...
        const idx = this._findKeyframeIndex(timeline, time, this.timelineIndices.players[playerName] || 0);
    ...
    updateFromFramework(player, currentTime) {
        ...
        if (ball && player.ball) {
            ...
            ball.visible = ballPositionValid && (ballState.visible !== false) && !ball.userData.isHiddenByGoal;
            ...
            if (this.ballVerticalLine) {
                ...
                const linePositions = new Float32Array([
                    ball.position.x, groundY, ball.position.z,
                    ball.position.x, ball.position.y, ball.position.z
                ]);
                this.ballVerticalLine.geometry.setAttribute('position', new THREE.BufferAttribute(linePositions, 3));
    ...
    processFrame(frame, getObjectName, frameIndex, isSeeking) {
            ...
            frame.new_actors.forEach(newActor => {
                if (!this.actors[newActor.actor_id]) {
                    ...
                    if (isBall || isCar) {
                        ...
                        const material = new THREE.MeshStandardMaterial({ color: isBall ? 0xffffff : Math.random() * 0xffffff });
                        const mesh = new THREE.Mesh(geometry, material);
        ...
        if (frame.updated_actors) {
            frame.updated_actors.forEach(update => {
                ...
                const isPRI = objectName && (objectName.includes('PRI_TA') || objectName.includes('PlayerReplicationInfo'));
                ...
                if (update.attribute.String && this.playerNames.has(update.attribute.String)) {
                    ...
                    if (isPRI && !this.playerNameToPriActorId[playerName]) {
                        ...
                        console.log(`[ActorManager] Mapped ${playerName} -> PRI Actor ${update.actor_id} (object: ${objectName})`);
                    }

                    this.checkCarPlayerLink(update.actor_id);
                }
                if (update.attribute.Reservation && this.playerNames.has(update.attribute.Reservation.name)) {
        ...
        if (this.effectsManager.explosions.goalEvents.has(frameIndex)) {
            ...
            if (ball) {
                if (!isSeeking) {
                    ...
                    console.log(`🎯 GOAL! Explosion at frame ${frameIndex} for team ${goalEvent.team} by ${goalEvent.playerName}`);
                }
        ...
        if (this.effectsManager.explosions.demoEvents.has(frameIndex)) {
            ...
            if (victimCar) {
                if (!isSeeking) {
                    ...
                    console.log(`💥 DEMO! Explosion at frame ${frameIndex} for actor ${demoEvent.victimActorId}`);
                }
    ...
    resolveBodyId(mesh, actorId) {
        ...
        if (mesh.userData.playerId && this.playerTeams.hasOwnProperty(mesh.userData.playerId)) {
    ...
    _doCarReplacement(actorId, oldMesh, carName, hitboxType) {
        ...
        let team = 0;
        if (oldMesh.userData.playerId && this.playerTeams.hasOwnProperty(oldMesh.userData.playerId)) {
        ...
        const modelType = this.carModelLoader.getModelTypeForCar(carName, hitboxType);
        console.log(`🚗 Replaced car ${actorId} with ${modelType.toUpperCase()} model (${carName}, ${hitboxType} hitbox, team ${team === 0 ? 'blue' : 'orange'})`);
    }
    ...
    updateInterpolation(time, frames, targetFrameIndex) {
        ...
        if (currentFrame) {
            Object.keys(this.actors).forEach(actorId => {
                ...
                for (let i = targetFrameIndex + 1; i < Math.min(frames.length, targetFrameIndex + 60); i++) {
                    const frame = frames[i];
                    if (frame.updated_actors) {
                        const update = frame.updated_actors.find(u => u.actor_id == actorId && u.attribute && u.attribute.RigidBody);
                        if (update) {
                            nextUpdate = update;
                ...
                Object.keys(this.actors).forEach(actorId => {
                    ...
                    if (actor && actor.userData.isCar && actor.userData.playerId) {
                        ...
                        if (distance < minDistance) {
                            ...
                            if (this.playerTeams.hasOwnProperty(playerName)) {
    ...
    setInterpolationMethod(method) {
        const validMethods = [
            'lerp', 'hermite', 'catmull-rom', 'predict-correct', 'velocity-smooth', 'physics-tick',
            'velocity-only', 'smart-hybrid', 'time-shifted', 'lerp-smooth', 'lerp-ema', 'lerp-dema', 'lerp-wma', 'lerp-gauss', 'one-euro',
            'position-lerp', 'position-catmull', 'position-smooth', 'adaptive-smooth'
        ];
    ...
    createCarMeshForLive(team, playerIndex, playerName, bodyId = null) {
        ...
        console.log(`[ActorManager] Created live car for ${playerName} (team ${team === 0 ? 'blue' : 'orange'}, bodyId: ${bodyId})`);
        return mesh;
    }
