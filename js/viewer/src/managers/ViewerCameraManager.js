...
export class ViewerCameraManager {
    ...
    async loadModel() {
        ...
        this.modelLoading = new Promise((resolve, reject) => {
            ...
            this.gltfLoader.load(
                ...
                (progress) => {
                    if (progress.total > 0) {
                        console.log(`[ViewerCameraManager] Loading: ${Math.round(progress.loaded / progress.total * 100)}%`);
                    }
                },
                (error) => {
    ...
    createLabel(nickname, color) {
        ...
        console.log(`[ViewerCameraManager] createLabel: nickname=${nickname}, inputColor=${color}, bgColor=${bgColor}`);
        ...
        console.log(`[ViewerCameraManager] Created label sprite for ${nickname} with color ${labelColor}`);
    ...
    async addOrUpdateViewer(participantId, participant) {
        ...
        if (!viewerData) {
            ...
            const shouldBeHidden = participantId === this.hiddenViewerId ||
            ...
            if (viewerData.mesh.visible !== !shouldBeHidden) {
                ...
                console.log(`[ViewerCameraManager] *** Visibility change: ${nickname} -> ${!shouldBeHidden ? 'VISIBLE' : 'HIDDEN'}`);
            }
        }
    ...
    updateViewerCamera(participantId, cameraState) {
        ...
        if (position) {
            ...
            const shouldBeHidden = participantId === this.hiddenViewerId ||
                                   this.hiddenFollowers.has(participantId);
            if (!shouldBeHidden) {
    ...
    updateFromParticipants(participants, selfId) {
        ...
        for (const id of newHiddenFollowers) {
            if (!this.hiddenFollowers.has(id)) {
                ...
                console.log(`[ViewerCameraManager] *** HIDING follower ${viewerData?.nickname || id} (mesh exists: ${!!viewerData})`);
