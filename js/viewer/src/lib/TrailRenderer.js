...
// Helper function to set update range (compatible with THREE.js r169+)
// The old updateRange API is deprecated, use addUpdateRange instead
function setUpdateRange(attribute, offset, count) {
    if (count === -1) {
        // -1 means update entire buffer
        attribute.clearUpdateRanges?.();
        attribute.addUpdateRange?.(0, attribute.count * attribute.itemSize);
    } else if (attribute.addUpdateRange) {
        // New API (THREE.js r169+)
        attribute.clearUpdateRanges();
...
export class TrailRenderer extends THREE.Object3D {
    ...
    constructor (scene, orientToMovement) {
        ...
        // Internal accumulated time for playback synchronization
        this._internalTime = 0;
        this._useInternalTime = false;
    }
    ...
    update(delta) {
        if (!this.paused) {
            // If delta is provided, use internal time accumulation
            if (delta !== undefined) {
                this._useInternalTime = true;
    ...
    updateNodeID (nodeIndex, id) { 
        ...
        setUpdateRange(nodeIDs, nodeIndex * this.VerticesPerNode, this.VerticesPerNode);
        setUpdateRange(nodeVertexIDs, nodeIndex * this.VerticesPerNode, this.VerticesPerNode);
    }
    ...
    updateNodeCenter (nodeIndex, nodeCenter) { 
        ...
        setUpdateRange(nodeCenters, nodeIndex * this.VerticesPerNode * TrailRenderer.PositionComponentCount, this.VerticesPerNode * TrailRenderer.PositionComponentCount);
    }
    ...
    updateNodePositionsFromTransformMatrix = function() { 
        ...
        return function updateNodePositionsFromTransformMatrix(nodeIndex, transformMatrix) {
            ...
            positions.needsUpdate = true;
            setUpdateRange(positions, nodeIndex * this.VerticesPerNode * TrailRenderer.PositionComponentCount, this.VerticesPerNode * TrailRenderer.PositionComponentCount);
        };
    ...
    static get LocalOrientationTangent () {
        return TrailRenderer._LocalOrientationTangent;
    }

    static _LocalHeadOrigin = new THREE.Vector3(0, 0, 0);
    static get LocalHeadOrigin () {
        return TrailRenderer._LocalHeadOrigin;
    }
