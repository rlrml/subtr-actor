...
| **NameTagManager** *(now `createNameTagPlugin()`)* | Floating player name labels | `beforeRender` | per-car transform + camera projection |
...
- **`ReplayLoader` / `framework/`** were a *second replay parser* (a JS boxcars
...
- Anything that loaded from a **backend API** (environments, assets, dev tools, collab
  sockets) must become *client-side or pluggable* to fit the "embeddable, fully
