...
test("renderTouchStats can break touches down by dodge state", () => {
  const html = renderTouchStats(
      ...
      labeled_touch_counts: {
        entries: [
          ...
          },
          {
            labels: [
              { key: "kind", value: "hard_hit" },
              { key: "dodge_state", value: "no_dodge" },
            ],
            count: 3,
          },
        ],
    ...
    {
      breakdownClasses: ["dodge_state"],
    },
  );

  assert.match(html, /No dodge<\/span><span class="value">3/);
  assert.match(html, /Dodge<\/span><span class="value">2/);
});
