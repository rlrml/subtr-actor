...
function advanceCeilingShotStats(
  ...
  stats.time_since_last_ceiling_shot =
    stats.last_ceiling_shot_time == null
      ? null
      : Math.max(0, frameTime - stats.last_ceiling_shot_time);
...
function applyCeilingShotEvent(
  ...
  stats.time_since_last_ceiling_shot = Math.max(0, frameTime - event.time);
...
export function applyCeilingShotEventDerivedStats(timeline: StatsTimeline): StatsTimeline {
