...
//!
//! This crate backs the `bakkesmod-tem-recorder/` BakkesMod plugin: the C++
...
//! each captured shot into a training-pack round (via the archetype-string
//! builders in [`archetypes`]) and serializes the pack with
...
//! a C header (`include/tem_recorder.h`) whose struct layouts are locked by
