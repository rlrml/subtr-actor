#!/usr/bin/env bash
# Linux MSVC-ABI cross build of the state-export plugin (clang-cl +
# lld-link + xwin sysroot), mirroring bakkesmod/replay-to-training/build-linux-msvc.sh
...
cargo build -p subtr-actor-state-export --release --target "$target"
popd >/dev/null
...
if [[ -f "$build_dir/StateExportPlugin.dll" ]]; then
  cp -f "$build_dir/StateExportPlugin.dll" "$plugin_out_dir/StateExportPlugin.dll"
elif [[ ! -f "$plugin_out_dir/StateExportPlugin.dll" ]]; then
  echo "missing built plugin DLL: $build_dir/StateExportPlugin.dll" >&2
  exit 1
fi
cp \
  "$repo_root/target/$target/release/state_export.dll" \
  "$plugin_out_dir/state_export.dll"

install_layout_dir="$plugin_out_dir/bakkesmod-install"
mkdir -p "$install_layout_dir/plugins" "$install_layout_dir/data/state-export"
cp -f \
  "$plugin_out_dir/StateExportPlugin.dll" \
  "$install_layout_dir/plugins/StateExportPlugin.dll"
cp -f \
  "$plugin_out_dir/state_export.dll" \
  "$install_layout_dir/data/state-export/state_export.dll"

echo "Built Linux MSVC-ABI state-export artifacts in $plugin_out_dir"
