...
for i in picks:
    raw = Xte[i]
    v = float(p_lr[i])
    vals = ", ".join(f"{float(x)!r}f32" for x in raw)
    fix.append(f"(&[{vals}], {v!r}f32),")
