...
impl BackboardCalculator {
    ...
    fn apply_event(&mut self, frame: &FrameInfo, event: &BackboardBounceEvent) {
        ...
        stats.frames_since_last_backboard = Some(frame.frame_number.saturating_sub(event.frame));

        self.team_stats_mut(event.is_team_0).count += 1;
        self.events.push(event.clone());
    }

    fn team_stats_mut(&mut self, is_team_0: bool) -> &mut BackboardTeamStats {
        if is_team_0 {
