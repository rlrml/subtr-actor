...
//! is a pair of small matrix-vector products over [`ThreatFeatures::to_array`].
//!
...
//! table has one named row per [`ThreatFeatures::FEATURE_NAMES`] entry, in the
//! same order.
...
//
// trained-v5 provenance: 8-hidden-unit tanh MLP fit by
...
// 2026-07-12). The newest 20% of replays were held out temporally: log_loss
// 0.13005, Brier 0.03456, AUC 0.8936, and 15-bin ECE 0.00129 (linear
// baseline log_loss 0.13548; GBT reference ceiling 0.12816). The published
// model was refit on the full corpus. Input standardization is folded into
// the first-layer weights, which apply directly to raw features.
// ---------------------------------------------------------------------------

/// Identifies the generated model embedded below.
pub const THREAT_MODEL_VERSION: &str = "trained-v5";
