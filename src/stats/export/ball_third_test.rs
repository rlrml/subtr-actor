...
#[test]
fn ball_third_export_includes_labeled_time_stats() {
    let mut stats = BallThirdStats {
        tracked_time: 4.0,
        ..Default::default()
    };
    stats
        .labeled_time
        .add([StatLabel::new("field_third", "team_zero_third")], 1.5);

    let labeled_stats: Vec<_> = stats
        .stat_fields()
        .into_iter()
        .filter(|stat| {
            stat.descriptor.domain == "ball_third"
    ...
    assert_eq!(
        ...
        vec![StatLabel::new("field_third", "team_zero_third")]
    );
