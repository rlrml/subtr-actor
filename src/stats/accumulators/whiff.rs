...
impl WhiffStats {
    ...
    pub(crate) fn record_whiff(&mut self, event: &WhiffEvent) {
...
impl WhiffStatsAccumulator {
    ...
    pub fn apply_event(&mut self, event: &WhiffEvent, frame: &FrameInfo) {
        ...
        match event.kind {
            WhiffEventKind::Whiff => {
                ...
                self.current_last_whiff_player = Some(event.player.clone());
            }
